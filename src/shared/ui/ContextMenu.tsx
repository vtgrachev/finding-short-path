...
export const ContextMenu: FunctionComponent<Props> = ({ open, positionTop, positionLeft, options = [] }) => {
    return (
        <ul
            className={`fixed p-2 bg-gray-50 rounded-[8px] border-gray-300 text-gray-900 sm:text-sm dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 divide-y transition-opacity opacity-0 ${open && 'opacity-100'}`}
